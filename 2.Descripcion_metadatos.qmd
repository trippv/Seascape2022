...
recood_geo_moll <- sf::st_transform(recood_geo, '+proj=moll')
...
ggplot(data = reworld)+
  geom_sf(color = "grey85", fill = "tan1",)+
  
  theme_bw()+
  geom_sf(data = recood_geo_moll, aes(col = n))+
# Add frequency grid data
  # geom_point(data = recood_geo_moll,
  #            aes(x = lon_round, y = lats_round, color = n), size = 2.5, pch = 15)+
...
main_driver_grid_recode_geo <- sf::st_transform(main_driver_grid_recode_geo, '+proj=moll')
#sf::st_is_longlat(recood_geo_moll)
